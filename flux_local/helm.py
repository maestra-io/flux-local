...
def get_helm_cache_dir() -> Path:
    """Get the Helm cache directory.

    Returns a Path to the Helm cache directory. The directory can be configured
    via the FLUX_LOCAL_HELM_CACHE_DIR environment variable. If not set, uses
    a persistent cache directory in the user's home directory.

    Returns:
        Path: The Helm cache directory path
    """
    cache_dir_env = os.environ.get("FLUX_LOCAL_HELM_CACHE_DIR")

    if cache_dir_env:
        cache_dir = Path(cache_dir_env)
        _LOGGER.debug("Using Helm cache directory from FLUX_LOCAL_HELM_CACHE_DIR: %s", cache_dir)
    else:
        # Use a persistent cache directory in user's home
        cache_dir = Path.home() / ".cache" / "flux-local" / "helm"
        _LOGGER.debug("Using default Helm cache directory: %s", cache_dir)
