...
class DiffHelmReleaseAction:
    ...
    async def run(  # type: ignore[no-untyped-def]
        ...
        **kwargs,  # pylint: disable=unused-argument
        ...
        from flux_local.helm import get_helm_cache_dir

        helm_cache_dir = get_helm_cache_dir()
        await asyncio.gather(
            helm_visitor.inflate(
                helm_cache_dir, helm_content.visitor(), options
            ),
            orig_helm_visitor.inflate(
                helm_cache_dir, orig_helm_content.visitor(), options
            ),
        )
